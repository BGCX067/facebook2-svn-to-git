var UploadProgress = {
  ...
  monitor: function(upid) {
    if(!this.periodicExecuter) {
      this.periodicExecuter = new PeriodicalExecuter(function() {
        ...
        new Ajax.Request('/lifebook/progress?upload_id=' + upid);
      }, 5);
    }
  ...
  update: function(total, current) {
    ...
    $('results').innerHTML  = "<small>已经上传 " + current.toHumanSize() + ' 总共 ' + total.toHumanSize() + " .</small>";
  ...
  processFile: function() {
    this.StatusBar.finish();
    $('results').innerHTML = '文件上传完成，正在处理文件...请稍后!';
  },
  ...
  FileField: {
    add: function() {
      new Insertion.Bottom('file-fields', '<p style="display:none"><input id="file" name="file" size="50" type="file" /> <a href="#" onclick="UploadProgress.FileField.remove(this);return false;">x</a></p>')
